...
<body>
    <section class="pt-5 pb-32">
          ...
          <div class="w-full px-4">
            <div class="max-w-xl mx-auto text-center mb-5">
                <h1 class="text-3xl text-blue-600/100 text-center font-bold">
                    Ubah Data Todolist (Pelajar)
                </h1>
            </div>
          </div>
          <form method="post" action="{{ url('update/'.$pelajar->id) }}">
            @csrf
            <div class="w-full lg:w-2/3 lg:mx-auto">
              <div class="w-full px-4 mb-4">
                <label for="nama" class="text-base font-bold text-blue-600/100">Nama</label>
                <input type="text" id="nama" name="nama" value="{{$pelajar->nama}}" class="w-full bg-slate-200 p-3 rounded-md" />
              </div>
              <div class="w-full px-4 mb-4">
                ...
                <select name="jk" class="w-full bg-slate-200 p-3 rounded-md">
                    ...
                    <option @if($pelajar['jk']=='Laki-Laki') selected @endif value="Laki-Laki">Laki-Laki</option>
					<option @if($pelajar['jk']=='Perempuan') selected @endif value="Perempuan">Perempuan</option>
                </select>
              </div>
              <div class="w-full px-4 mb-4">
                ...
                <select name="jurusan_id" class="w-full bg-slate-200 p-3 rounded-md">
                    ...
                    @foreach ($jurusans as $jurusan)
                        <option value="{{ $jurusan->id }}" {{ $jurusan->id == $pelajar->jurusan_id ? 'selected' : '' }}>{{ $jurusan->jurusan }} / 
                            {{ ($jurusan->kelas) }}
                        </option>
                    @endforeach
                </select>
              ...
              <div class="w-full px-4 mb-4">
                <div class="input-group">
                    ...
                    <input type="text" id="hobi" name="hobi[]" value="{{$pelajar->hobis[0]->hobi ?? '' }}" class="w-full bg-slate-200 p-3 rounded-md" />
      ...
      <script>
        ...
        add.forEach(function(e){
            e.addEventListener('click',function(){
                ...
                let newElement = document.createElement('div')
                    ...
                    newElement.innerHTML = `<input type="text" name="hobi[]" class="pt-5 w-full bg-slate-200 p-3 rounded-md" />
                    <button type="button" class="w-full inline-block px-6 py-2.5 bg-red-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-red-700 hover:shadow-lg focus:bg-red-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-red-800 active:shadow-lg transition duration-150 ease-in-out remove_hobi">Hapus</button>`
        ...
        function callEvent(){
            document.querySelector('form').querySelectorAll('.remove_hobi').forEach(function(remove){
